...
RUN echo "Installing Mariadb" && apk add --no-cache mariadb mariadb-client mariadb-server-utils bash vim && mkdir -vp $DB_SOCKET_PATH && chown mysql:mysql $DB_SOCKET_PATH && mysql_install_db --user=mysql --datadir=$DB_DATA_PATH && mysqld_safe --datadir=$DB_DATA_PATH  --no-auto-restart && mysqladmin create $DB_NAME && mysqladmin create $DB_NAME && echo "GRANT ALL PRIVILEGES ON $DB_NAME.* TO '$DB_USER'@'localhost' IDENTIFIED BY '$DB_PASS';" > /tmp/create_user.sql && mysql < /tmp/create_user.sql && mysqladmin --user=root password $DB_ROOT_PASS && pkill mysql
EXPOSE $DB_PORT
